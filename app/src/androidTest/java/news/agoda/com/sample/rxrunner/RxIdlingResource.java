package news.agoda.com.sample.rxrunner;
...
public class RxIdlingResource implements IdlingResource {
    ...
    public void incrementActiveSubscriptionsCount() {
        ...
        Loge.i("Active subscriptions count increased to " + count);
    }

    public void decrementActiveSubscriptionsCount() {
        int count = mActiveSubscriptionsCount.decrementAndGet();
        Loge.i("Active subscriptions count decreased to " + count);
        if (isIdleNow()) {
            Loge.i("There is no active subscriptions, transitioning to Idle");
